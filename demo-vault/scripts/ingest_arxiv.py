...
USER_AGENT = "delta-force-vault/1.0 (+local research archive)"
...
def _collision_basename(title_slug: str, ref: ArxivRef) -> str:
    safe_id = ref.paper_id.replace("/", "-")
    return f"{title_slug}-arxiv-{safe_id}-v{ref.version}"


def _select_basename(
    raw_dir: Path, title_slug: str, ref: ArxivRef
) -> str:
    ordinary = raw_dir / f"{title_slug}.md"
    ordinary_pdf = raw_dir / "assets" / f"{title_slug}.pdf"
    if not ordinary.exists() and not ordinary_pdf.exists():
        return title_slug
    if _existing_ref(ordinary) == ref:
        return title_slug

    collision = _collision_basename(title_slug, ref)
    collision_md = raw_dir / f"{collision}.md"
    collision_pdf = raw_dir / "assets" / f"{collision}.pdf"
...
def ingest_paper(
    ...
    *,
    ...
    basename = _select_basename(raw_dir, title_slug, ref)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = ingest_paper(
            arguments.vault, parse_arxiv_ref(arguments.arxiv_id)
        )

...
* **Checkout-safe validation**: Made ignored `.llm_wiki/` state and registered
  `projects/code/<id>/` working copies opaque to vault validation, while still
  rejecting tracked runtime state; empty project asset stores now become
  concrete only when their first LFS binary is added.

...
class IngestArxivTests(unittest.TestCase):
    ...
    def test_ingest_publishes_matching_companion_and_pdf_atomically(self) -> None:
        ...
        self.assertEqual(document.metadata["attachment"]["role"], "original")
        self.assertEqual(
    ...
    def test_ingest_defaults_to_outer_raw_and_assets(self) -> None:
        result = ingest_paper(
            ...
            pdf_loader=pdf_loader_with(b"%PDF-1.7\nouter fixture\n"),
            extractor=successful_extractor,
            ingested_date=date(2026, 8, 13),
        )
        self.assertEqual(result.markdown_path.parent.resolve(), (self.root / "raw").resolve())
        self.assertEqual(result.pdf_path.parent.resolve(), (self.root / "assets").resolve())
    ...
    def test_title_collision_gets_deterministic_digest_suffix(self) -> None:
        first = self.ingest(ref=ArxivRef("9999.99999", 1))
        second_pdf = b"%PDF-1.7\nsecond\n"
        second = self.ingest(ref=ArxivRef("1508.07909", 5), pdf_bytes=second_pdf)
        suffix = sha256_bytes(second_pdf)[:12]

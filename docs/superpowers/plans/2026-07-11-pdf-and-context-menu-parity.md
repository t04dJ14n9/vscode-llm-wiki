...
**Interfaces:**
- Consumes: PDF `Insert link`, persisted `llm-wiki://pdf/...` anchors, Markdown link activation, PDF referenced-highlight popover, and `openMarkdownAtLocation`.
...
- [ ] **Step 1: Create unique Markdown and PDF artifacts**
  ...
  Create a Markdown note whose first line is `# Link Round Trip 2026-07-12` and a PDF containing the unique sentence `LLM Wiki round-trip anchor 2026-07-12.` Open both through the Extension Development Host, not through source-editor fallbacks.
...
- [ ] **Step 2: Insert a persisted PDF-selection link into Markdown**
  ...
  Select the unique PDF sentence, open the PDF++-style secondary-click menu, choose `Insert link`, and save the Markdown note. Verify the note contains one Markdown link with a `llm-wiki://pdf/` destination and that the PDF selection becomes a referenced highlight after indexing.

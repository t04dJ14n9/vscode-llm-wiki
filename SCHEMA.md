...
The repository root is the OKF v0.2 bundle root and unit of distribution.
Markdown in the bundle-owned knowledge directories is concept data; engineering
directories such as `packages/`, `tools/`, and `docs/` are outside the bundle
graph. `index.md` and `log.md` are reserved at every bundle-owned level.

The reusable skill and producer scripts live in `.agents/` and `tools/`,
outside the bundle-owned knowledge directories. `projects/code/nanochat/` is
an opaque nested Git repository and is not parsed as OKF.

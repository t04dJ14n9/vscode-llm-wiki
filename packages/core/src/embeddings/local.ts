...
export async function refreshEmbeddings(
  ...
  const useRemote = embCfg?.mode === 'remote' &&
    (embCfg.provider === 'ollama' || embCfg.provider === 'openai-compatible') &&
    embCfg.model && embCfg.base_url;

  const modelId = options.modelId ?? (useRemote ? (embCfg!.model ?? 'remote') : LOCAL_EMBEDDING_MODEL);
  ...
  for (const chunk of chunks) {
    ...
    if (useRemote) {
      try {
        vector = await embedTextRemote(chunk.text, {
          provider: embCfg!.provider as 'ollama' | 'openai-compatible',
          model: embCfg!.model!,

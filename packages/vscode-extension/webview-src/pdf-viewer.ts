...
class PdfViewer {
  ...
  private async layoutPages(): Promise<void> {
    ...
    for (let index = 0; index < pageCount; index++) {
      ...
      wrapper.style.width = `${Math.floor(pageObj.size.width * this.scale)}px`;
      wrapper.style.height = `${Math.floor(pageObj.size.height * this.scale)}px`;
  ...
  private async renderPage(pageNum: number): Promise<void> {
    ...
    try {
      ...
      const blob: Blob = await engine
        .renderPage(pdfDoc, state.pageObj, { scaleFactor: this.scale, dpr, withAnnotations: true })
        .toPromise();
      ...
      await new Promise<void>((resolve, reject) => {
        image.onload = () => {
          ...
          state.canvas.style.width = `${Math.floor(state.pageObj.size.width * this.scale)}px`;
          state.canvas.style.height = `${Math.floor(state.pageObj.size.height * this.scale)}px`;
          state.canvas.getContext('2d')!.drawImage(image, 0, 0);
      ...
      rects.forEach((item, itemIndex) => {
        ...
        span.style.left = `${item.rect.origin.x * this.scale}px`;
        span.style.top = `${item.rect.origin.y * this.scale}px`;
        span.style.width = `${item.rect.size.width * this.scale}px`;
        span.style.height = `${item.rect.size.height * this.scale}px`;
        span.style.lineHeight = `${item.rect.size.height * this.scale}px`;
        span.style.fontSize = `${Math.max(1, item.rect.size.height * this.scale)}px`;
  ...
  private async rerender(): Promise<void> {
    for (const page of this.pages.values()) {
      page.rendered = false;
      page.wrapper.style.width = `${Math.floor(page.pageObj.size.width * this.scale)}px`;
      page.wrapper.style.height = `${Math.floor(page.pageObj.size.height * this.scale)}px`;
